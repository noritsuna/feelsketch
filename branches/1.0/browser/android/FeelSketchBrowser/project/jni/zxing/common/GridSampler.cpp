...
namespace common {
  ...
  int GridSampler::sampleGrid(Ref<MonochromeBitmapSource> image,
    ...
    for (int i = 0; i < dimension; i++) {
      ...
      int error = checkAndNudgePoints(image, points);
      if (error != 0) {
    	  return error;
      }
      for (int j = 0; j < max; j += 2) {
    	  resultPoints->set(j/2, i, (int)points[j], (int)points[j+1]);
      }
    }
  ...
  int GridSampler::checkAndNudgePoints(Ref<MonochromeBitmapSource> image,
